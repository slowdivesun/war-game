    ...
    def hit(self, rect):
        ...
        def overlaps(item):
            return (
                ...
                and rect.left <= item.right
                and rect.bottom >= item.top
                and rect.top <= item.bottom
            )
...
class QuadTree(object):
    ...
    def __init__(self, items, depth=8, bounding_rect=None):
        ...
        if nw_items:
            self.nw = QuadTree(
                nw_items, depth, (bounding_rect.left, bounding_rect.top, cx, cy)
            )
        if ne_items:
            self.ne = QuadTree(
                ne_items, depth, (cx, bounding_rect.top, bounding_rect.right, cy)
            )
        if se_items:
            self.se = QuadTree(
                se_items, depth, (cx, cy, bounding_rect.right, bounding_rect.bottom)
            )
        if sw_items:
            self.sw = QuadTree(
                sw_items, depth, (bounding_rect.left, cy, cx, bounding_rect.bottom)
            )
